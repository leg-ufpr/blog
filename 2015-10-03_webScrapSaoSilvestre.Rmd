...
informação verbal. Quando os dados estão reunidos dentro de um único
arquivo não existe problema de coleta. Você baixa esse arquivo e começa
a processar e análisar de imediato. Por outro lado, muitos dados estão
disponíveis de uma maneira fragmentada e a coleta deles pode ser um
desafio.
...
com tabelas de 10 linhas, uma por atleta. Com os dados fragmentados
desse jeito, qualquer iniciativa do tipo recorta-cola-edita-repete, algo
que muitos usuários de planilha eletrônica fazem todos os dias, é
abandonada por ser impraticável, inexequível.

Vou partir para uma solução computacional, afinal, o computador tem que
trabalhar para mim, e não eu para ele. Vou trabalhar com técnicas de
web scraping para extrair esse resultados de todas as páginas.

As páginas de resultado têm uma estrutura fixa que é mantida em todas.
Os resultados para o masculino de [2014] consistem de cabeçalho e rodapé
com informações de situação, botões para mudar de página, e a tabela que
quero ao centro. Essa têm 10 linhas e 8 colunas que incluem informações
...
## Url para a 4 página dos resultados do masculino em 2014.
...
url <- paste0("http://www.yescom.com.br",
              ...
              "?evento_yescom_id=1612&tipo=3",
              "&tipo_do_evento_id=4243",
              "&PaginaAtual=4",
...
pedi classe (`class`) e depois dimensão (`dim`). Nisso ví que tinha um
...
são informações que vieram do inspecionar elementos que fiz.
...
para chegar à espeficicação abaixo.
...
fazia a função. No laço `for` que faz a extração dos dados, eu incluí um
indicador de progresso modo texto.
...
`tipo=4` e `tipo=3`. Independente do sexo, as urls têm
...
url0 <- paste0("http://www.yescom.com.br",
               "/codigo_comum/classificacao/codigo",
               "/p_classificacao03_v1.asp?",
               "evento_yescom_id=1511",
               "&tipo=%d",        ## <-- indica o sexo
               "&tipo_do_evento_id=4128&",
               "PaginaAtual=%d",  ## <-- %d para página
               "&faixa=&sexo=M",  ## <-- %s para sexo
               "&campo=&pesquisa=")
...
com a internet. O processo levou 30 minutos.
...
dtfr <- read.table("saoSilvestre2014.csv",
                   header=FALSE, sep=";",
                   stringsAsFactors=FALSE)
...
2007 e que mantiveram o padrão estrutural. Por outro lado, antes de
2007, eles eram divulgados em formato texto de comprimento fixo e cada
ano tinha uma estrutura diferente. Para ler os últimos, você pode usar a
`read.fwf()`.

[São Silvestre]: http://www.saosilvestre.com.br/resultados/
[2014]: http://www.yescom.com.br/codigo_comum/classificacao/codigo/p_classificacao03_v1.asp?evento_yescom_id=1612&tipo=3&tipo_do_evento_id=4243&PaginaAtual=1&faixa=&sexo=M&campo=&pesquisa=
