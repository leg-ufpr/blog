...
##======================================================================
## Web scraping em cifras de música
## http://blog.leg.ufpr.br/archives/195
...
permite minerar páginas html. É um pacote oficial do R, então você instala
com um simples `install.packages("XML")`.
...
url <- "http://www.cifraclub.com.br/chico-buarque/"
## browseURL(url)
...
## OBS: Não peça para ver esse objeto no seu console. Geralmente páginas
## html são extensas. Baixe a página ou envie para um arquivo texto e
## abra em um editor de texto de sua preferência.

## cat(con=pr, file="CBH.html")                ## Salva em arquivo.
## download.file(url=url, destfile="CBH.html") ## Baixa.
...
legível. Em poucas palavras, um documento html contituído dessas tags e
...
inspecionar elemento do seu navagador. Clique com o botão direito sobre a
página e escolha **inspecionar elemento**. Na janela que se abrir, você
pode navegar pelo código fonte e ver que parte do código fonte
corresponde ao que é mostrado no navegador. O *printscreen* abaixo
esclarece como fiz a minha especifiação. A numeração na figura indica as
etapas pelas quais passei.
...
![](http://blog.leg.ufpr.br/~walmes/cifrasChicoBuarqueHome.png)
...
## Use a ferramentar inspecionar elemento do seu navegador para ver que
## a lista de canções está dentro da tag:
##   <ul class="art_musics js-art_songs" data-total-songs="">
## Cada cançé um elemento da lista dentro da tag <li>.

## O nome de cada canção.
nc <- getNodeSet(
    doc=h,
    ## path="//ul[@class]",
    ## path="//ul[@class=\"art_musics js-art_songs\"]",
    ## path="//ul[@class=\"art_musics js-art_songs\"]//li",
    ## path="//ul[@class=\"art_musics js-art_songs\"]//li[@class=\"js-art_song \"]",
    ## path="//ul[@class=\"art_musics js-art_songs\"]//li[@class=\"js-art_song \"]//a",
...
## REGEX que remove os espaços nas extremidades.
nc <- gsub(x=nc, pattern="^\\s+(.*\\S)\\s+$", replacement="\\1")
## str(nc)
...
head(data.frame(cancoes=nc, links=lc))

```

Agora que temos em um objeto com os links das canções (`lc`), já podemos
passar para etapa de extrair os acordes de cada uma delas. Vamos mostar
o *web scraping* para uma página com música cifrada. A canção escolhida
foi a [trocando um miúdos][]. Eu particularmente gosto da versão da
[Alcione com o Emílio Santiago][]. O *printscreen* que orientou minha
consulta e os números indicam as etapas.
  ...
  3. Reconheci que estava dentro da tag `<b>` (*bold*) e que tinha uma
     classe vazia.
  4. Vi que todo o texto da canção estava dentro da tag `<pre>` (*preformatted*).
...
![](http://blog.leg.ufpr.br/~walmes/cifrasChicoBuarqueTrocandoEmMiudos.png)

```{r}
##----------------------------------------------------------------------
## Lendo uma canção para contar o número de acordes que aparecem.

i <- "http://www.cifraclub.com.br/chico-buarque/trocando-em-miudos/"
## browseURL(i)

rl <- readLines(con=i, encoding="utf-8")
h <- htmlTreeParse(rl, asText=TRUE,
...
d <- getNodeSet(doc=h,
                ## path="//div",
                ## path="//div[@class]",
                ## path="//div[@class='cifra_cnt cifra-mono']",
                ## path="//div[@class='cifra_cnt cifra-mono']//pre",
...
urls <- paste0("http://www.cifraclub.com.br", lc)
...
## Algumas páginas dão 0 acordes devido estrutura
## diferente. Eliminar. Uma canção tem que ter no mínimo dois acordes.
...
scraping*. Baixe o arquivo R com os códigos mostrados nesse post: [script.R][].
...
## Nome do arquivo.
post <- "2015-08-26_webScrapChico.Rmd"
